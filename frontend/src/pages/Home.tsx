...
export const Home: React.FC = () => {
  ...
  const [segmentDetails, setSegmentDetails] = useState<DictionaryResponse|null>(null);
  ...
  const handleTokenSelect = async (token: Token) => {
    try {
      ...
      setError({
        message: err instanceof Error ? err.message: "Failed to fetch definition",
  ...
  const fetchDefinition = async (sentence: string, word: string) : Promise<DictionaryResponse> => {
    const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/deepseek/define`, {
      method: 'POST',
  ...
  return (
    <div className='p-4'>
      ...
      {tokens.length > 0 && (
        <>
            <TokenDisplay tokens={tokens} selected={selectedToken} onSelect={handleTokenSelect} />
